...
let contenu: string
let verifiecateurPerso : boolean = true;
...
export function creationPerssonage() {
  for (const iteration of dataHero) {
    if (iteration.rarity == getRandomInt(5) ) {
      hero = {
        name: iteration.name,
        hp: iteration.hp,
        str: iteration.str,
        rarity: iteration.rarity
      }
...
export function combat(round: number,  isNextFloorUnlocked: boolean, currentFloor : number) {
  ...
  if(currentFloor != compteur && isNextFloorUnlocked == verifiecateurPerso ){
  creationPerssonage();
  compteur++;
  verifiecateurPerso = false
...
if (currentFloor == 9) {
  affichageBar(boss.hp, bossHPMax, boss.name)
   ...
   if(currentFloor == 9){ const resume =  "You attacked and dealt " + hero.str + " Damages! \nYou receive " + boss.str+ " Damages"
   tableau(resume);}
   else {const resume =  "You attacked and dealt " + hero.str + " Damages! \nYou receive " + monstre.str+ " Damages"
   tableau(resume);}
